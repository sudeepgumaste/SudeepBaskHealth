...
- layout.tsx:
  The project features a layout that serves as a container for the dashboard and the widgets. The main advantage of using the layout is that if there's an api request required to render the page UI, next doesn't wait for the async data request and responds with the html that's not blocked by the network requests. It also ensures that if there are anymore routes added, the layout remains consistent.

- loading.tsx:
  This works in conjunction with react's suspense component internally. The structure provided here goes in place of the UI that needs to be generated after the api request on the server side is resolved.

- html streaming:
  Now that next has responded with layout and the loading state, it waits for the network request to complete. Once the request is resolved, the UI is generated on the server side and streamed to the client.

- error.tsx:
  This is used to handle and errors while generating the UI that relies on some api request to resolve. In case of an error, the structure provided in this file is rendered.
...
We have discussed in the [next.js features used](#nextjs-features-used) section about how the server side errors have been handled. And on the client side, as stated before, the project uses `@tanstack/react-query` for data fetching. The api errors are handled gracefully by reattempting the request on failure. In case all the reattempts fail(5 retries by default), I've added a toast notification to notify the user that something went wrong with the request.
